...
- (void)setShowRoutingButton:(BOOL)arg1 {
	...
	if ([controller respondsToSelector:@selector(context)] && controller.context == 2 && isRoutingButtonHidden) {
		arg1 = NO;
	}
	%orig(arg1);
}
...
- (void)didMoveToSuperview {
	...
	if (controller.context == 2 && configurations == 0) {
		...
		if (!artistNameLabel) { //creates the label with whatever settings
			...
			[artistNameLabel setFont:[UIFont systemFontOfSize:15.0f]];
		...
		[self.transportControlsView setFrame:CGRectMake(CGRectGetMidX(self.headerView.artworkView.frame) + 5,CGRectGetMidY(self.headerView.frame), self.transportControlsView.frame.size.width, s elf.transportControlsView.frame.size.height)];
...
>>>>>>> c1eca367c3f19e9732375c195a12b834156646fc
}
